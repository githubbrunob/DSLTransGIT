 ...
 * contains <code>mprolog.resource.pl.IPlTextDiagnostic</code>s it uses the more
 * precise information of this extended diagnostic type.
 */
public class PlMarkerHelper {
	
	/**
	 * The extension id of the custom marker type that is used by this text resource.
	 */
	public static final String MARKER_TYPE = mprolog.resource.pl.mopp.PlPlugin.PLUGIN_ID + ".problem";
	...
	private static class MarkerCommandQueue {
		...
		private java.util.List<mprolog.resource.pl.IPlCommand<Object>> commands = new java.util.ArrayList<mprolog.resource.pl.IPlCommand<Object>>();
		private final Object jobLock = new Object();
		
		public void addCommand(mprolog.resource.pl.IPlCommand<Object> command) {
			...
			new org.eclipse.core.runtime.jobs.Job("updating markers") {
				...
				protected org.eclipse.core.runtime.IStatus run(org.eclipse.core.runtime.IProgressMonitor monitor) {
					synchronized(jobLock) {
						java.util.List<mprolog.resource.pl.IPlCommand<Object>> commandsToProcess = new java.util.ArrayList<mprolog.resource.pl.IPlCommand<Object>>();
						synchronized(commands) {
							commandsToProcess.addAll(commands);
							commands.clear();
						}
						for (mprolog.resource.pl.IPlCommand<Object> command : commandsToProcess) {
	 ...
	 */
	public static void mark(org.eclipse.emf.ecore.resource.Resource resource, final mprolog.resource.pl.IPlTextDiagnostic diagnostic) {
	...
	private static void createMarkerFromDiagnostic(final org.eclipse.core.resources.IFile file, final mprolog.resource.pl.IPlTextDiagnostic diagnostic) {
		final mprolog.resource.pl.IPlProblem problem = diagnostic.getProblem();
		mprolog.resource.pl.PlEProblemType problemType = problem.getType();
		final String markerID = getMarkerID(problemType);
		COMMAND_QUEUE.addCommand(new mprolog.resource.pl.IPlCommand<Object>() {
			public boolean execute(Object context) {
				try {
					...
					if (problem.getSeverity() == mprolog.resource.pl.PlEProblemSeverity.ERROR) {
					...
					mprolog.resource.pl.IPlTextDiagnostic textDiagnostic = (mprolog.resource.pl.IPlTextDiagnostic) diagnostic;
					...
					if (diagnostic instanceof mprolog.resource.pl.mopp.PlResource.ElementBasedTextDiagnostic) {
						org.eclipse.emf.ecore.EObject element = ((mprolog.resource.pl.mopp.PlResource.ElementBasedTextDiagnostic) diagnostic).getElement();
					...
					java.util.Collection<mprolog.resource.pl.IPlQuickFix> quickFixes = textDiagnostic.getProblem().getQuickFixes();
					...
					if (!sourceIDs.isEmpty()) {
						marker.setAttribute(org.eclipse.core.resources.IMarker.SOURCE_ID, mprolog.resource.pl.util.PlStringUtil.explode(sourceIDs, "|"));
					}
	...
	public static void unmark(org.eclipse.emf.ecore.resource.Resource resource) {
		for (mprolog.resource.pl.PlEProblemType nextType : mprolog.resource.pl.PlEProblemType.values()) {
	 ...
	 */
	public static void unmark(org.eclipse.emf.ecore.resource.Resource resource, mprolog.resource.pl.PlEProblemType problemType) {
		final org.eclipse.core.resources.IFile file = getFile(resource);
		if (file == null) {
			return;
		}
	...
	public static void unmark(org.eclipse.emf.ecore.resource.Resource resource, final org.eclipse.emf.ecore.EObject causingObject) {
		...
		final String markerID = getMarkerID(mprolog.resource.pl.PlEProblemType.UNKNOWN);
	 ...
	 * given type.
	 */
	private static String getMarkerID(mprolog.resource.pl.PlEProblemType problemType) {
	...
	private static void handleException(org.eclipse.core.runtime.CoreException ce) {
		if (ce.getMessage().matches("Marker.*not found.")) {
			...
			mprolog.resource.pl.mopp.PlPlugin.logError("Error while removing markers from resource:", ce);
		}
	}
}
