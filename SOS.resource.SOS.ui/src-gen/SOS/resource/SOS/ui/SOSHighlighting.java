 ...
 */
public class SOSHighlighting implements org.eclipse.jface.viewers.ISelectionProvider, org.eclipse.jface.viewers.ISelectionChangedListener {
	
	private final static SOS.resource.SOS.ui.SOSPositionHelper positionHelper = new SOS.resource.SOS.ui.SOSPositionHelper();
	...
	private SOS.resource.SOS.ui.SOSTokenScanner scanner;
	...
	private SOS.resource.SOS.ui.SOSEditor editor;
	private org.eclipse.jface.text.source.projection.ProjectionViewer projectionViewer;
	private SOS.resource.SOS.ui.SOSOccurrence occurrence;
	private SOS.resource.SOS.ui.SOSBracketSet bracketSet;
	 ...
	 */
	private final class UpdateHighlightingListener implements org.eclipse.swt.events.KeyListener, org.eclipse.swt.events.VerifyListener, org.eclipse.swt.events.MouseListener, SOS.resource.SOS.ISOSBackgroundParsingListener {
	 ...
	 * @param editor
	 */
	public SOSHighlighting(SOS.resource.SOS.ISOSTextResource textResource, org.eclipse.jface.text.source.projection.ProjectionViewer sourceviewer, SOS.resource.SOS.ui.SOSColorManager colorManager, SOS.resource.SOS.ui.SOSEditor editor) {
		this.display = org.eclipse.swt.widgets.Display.getCurrent();
		sourceviewer.getSelectionProvider();
		preferenceStore = SOS.resource.SOS.ui.SOSUIPlugin.getDefault().getPreferenceStore();
		this.editor = editor;
		textWidget = sourceviewer.getTextWidget();
		projectionViewer = sourceviewer;
		scanner = new SOS.resource.SOS.ui.SOSTokenScanner(textResource, colorManager);
		occurrence = new SOS.resource.SOS.ui.SOSOccurrence(textResource, sourceviewer, scanner);
		bracketSet = new SOS.resource.SOS.ui.SOSBracketSet(editor, sourceviewer);
		this.colorManager = colorManager;
		isHighlightBrackets = preferenceStore.getBoolean(SOS.resource.SOS.ui.SOSPreferenceConstants.EDITOR_MATCHING_BRACKETS_CHECKBOX);
		bracketColor = colorManager.getColor(org.eclipse.jface.preference.PreferenceConverter.getColor(preferenceStore, SOS.resource.SOS.ui.SOSPreferenceConstants.EDITOR_MATCHING_BRACKETS_COLOR));
		black = colorManager.getColor(new org.eclipse.swt.graphics.RGB(0, 0, 0));
		
		addListeners(editor);
	}
	
	private void addListeners(SOS.resource.SOS.ui.SOSEditor editor) {
	...
	private void setBracketHighlighting(org.eclipse.jface.text.IDocument document) {
		...
		org.eclipse.jface.text.Position[] positions = positionHelper.getPositions(document, SOS.resource.SOS.ui.SOSPositionCategory.BRACKET.toString());
	...
	private void removeHighlighting() {
		...
		removeHighlightingCategory(document, SOS.resource.SOS.ui.SOSPositionCategory.BRACKET.toString());
	}
	...
	private void removeHighlightingCategory(org.eclipse.jface.text.IDocument document, String category) {
		...
		if (category.equals(SOS.resource.SOS.ui.SOSPositionCategory.BRACKET.toString())) {
		...
		display.syncExec(new Runnable() {
			public void run() {
				...
				if (selectedEObject != null) {
					setSelection(new SOS.resource.SOS.ui.SOSEObjectSelection(selectedEObject, false));
				}
			}
		});
	}
