...
public class Tester {
	...
	public static void main(String[] args) {
		...
		cleanTestsDirOutputs(testsDir);

		runTests(testsDir);
	}

	private static void cleanTestsDirOutputs(File testsDir) {
		File[] testSubDirectories = getIndividualTestDirs(testsDir);

		assert testSubDirectories != null;
		
		for (File testDir : testSubDirectories) {
			deleteOutputs(testDir);
		}
	}

	private static void deleteOutputs(File testDir) {
		...
		File[] outputFiles = testDir.listFiles(fileFilter);
		...
		for (File outputFile : outputFiles) {
			...
			for (Delta delta : deltas) {
				System.err.println("##### " + testDir.getAbsolutePath() + ": "
						+ outputFile.getName() + " <-->  "
						+ correspondingCorrectFile.getName() + " #####");
	...
	private static void runTransformation(File transformation) {
		try {
			...
			System.err.println("Error running transformation: "
					+ transformation.getAbsolutePath());
			e.printStackTrace();
		}
	}
	...
	private static File getCorrectModel(File testDir, final File outputFile)
			throws IOException {
		assert testDir.isDirectory();
		...
		if (result.length != 1) {
			throw new RuntimeException(
					"Too many or none correct output models found: "
							+ testDir.getCanonicalPath());
		}
	...
	private static void deleteRecursively(File f) throws IOException {
		...
		if (!f.delete())
			System.err
					.println("Failed to delete file: " + f.getCanonicalPath());
	}
