...
public class DsltranstextDefaultResolverDelegate<ContainerType extends org.eclipse.emf.ecore.EObject, ReferenceType extends org.eclipse.emf.ecore.EObject> {
	...
	private static class ReferenceCache implements dsltrans.resource.dsltranstext.IDsltranstextReferenceCache, org.eclipse.emf.common.notify.Adapter {
	 ...
	 */
	protected void resolve(String identifier, ContainerType container, org.eclipse.emf.ecore.EReference reference, int position, boolean resolveFuzzy, dsltrans.resource.dsltranstext.IDsltranstextReferenceResolveResult<ReferenceType> result) {
		try {
			org.eclipse.emf.ecore.EObject root = container;
			if (!enableScoping) {
				root = dsltrans.resource.dsltranstext.util.DsltranstextEObjectUtil.findRootContainer(container);
			}
			while (root != null) {
	...
	private java.util.Set<org.eclipse.emf.ecore.EObject> findReferencedExternalObjects(org.eclipse.emf.ecore.EObject object) {
		org.eclipse.emf.ecore.EObject root = dsltrans.resource.dsltranstext.util.DsltranstextEObjectUtil.findRootContainer(object);
	 ...
	 */
	protected boolean tryToResolveIdentifierInObjectTree(String identifier, org.eclipse.emf.ecore.EObject container, org.eclipse.emf.ecore.EObject root, org.eclipse.emf.ecore.EReference reference, int position, boolean resolveFuzzy, dsltrans.resource.dsltranstext.IDsltranstextReferenceResolveResult<ReferenceType> result, boolean checkRootFirst) {
	...
	private boolean tryToResolveIdentifierAsURI(String identifier, ContainerType container, org.eclipse.emf.ecore.EReference reference, int position, boolean resolveFuzzy, dsltrans.resource.dsltranstext.IDsltranstextReferenceResolveResult<ReferenceType> result) {
		org.eclipse.emf.ecore.EClass type = reference.getEReferenceType();
	...
	private boolean checkElement(org.eclipse.emf.ecore.EObject container, org.eclipse.emf.ecore.EObject element, org.eclipse.emf.ecore.EReference reference, int position, org.eclipse.emf.ecore.EClass type, String identifier, boolean resolveFuzzy, boolean checkStringWise, dsltrans.resource.dsltranstext.IDsltranstextReferenceResolveResult<ReferenceType> result) {
		...
		if (exactMatch == null) {
			...
			if (similarMatch != null) {
				...
				result.addQuickFix(new dsltrans.resource.dsltranstext.mopp.DsltranstextChangeReferenceQuickFix("Replace with " + similarMatch, "IMG_TOOL_FORWARD", container, reference, oldTarget, element));
			}
			return true;
		}
	...
	protected String produceDeResolveErrorMessage(org.eclipse.emf.ecore.EObject refObject, org.eclipse.emf.ecore.EObject container, org.eclipse.emf.ecore.EReference reference, dsltrans.resource.dsltranstext.IDsltranstextTextResource resource) {
		...
		if (nameAttr instanceof org.eclipse.emf.ecore.EAttribute) {
			...
			for (org.eclipse.emf.ecore.EOperation operation : element.eClass().getEAllOperations()) {
				if (operation.getName().toLowerCase().endsWith(NAME_FEATURE) && operation.getEParameters().size() == 0) {
					String result = (String) dsltrans.resource.dsltranstext.util.DsltranstextEObjectUtil.invokeOperation(element, operation);
					names.add(result);
				}
			}
		}
	...
	protected dsltrans.resource.dsltranstext.IDsltranstextReferenceCache getCache(org.eclipse.emf.ecore.EObject object) {
		org.eclipse.emf.ecore.EObject root = dsltrans.resource.dsltranstext.util.DsltranstextEObjectUtil.findRootContainer(object);
	...
	private boolean isSimilar(String identifier, Object attributeValue) {
		if (attributeValue != null && attributeValue instanceof String) {
			String name = (String) attributeValue;
			if (dsltrans.resource.dsltranstext.util.DsltranstextStringUtil.computeLevenshteinDistance(identifier, name) <= MAX_DISTANCE) {
				return true;
			}
		}
		return false;
