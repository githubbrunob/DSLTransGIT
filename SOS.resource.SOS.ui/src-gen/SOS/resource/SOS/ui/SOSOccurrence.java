 ...
 */
public class SOSOccurrence {
	
	public final static String OCCURRENCE_ANNOTATION_ID = "SOS.resource.SOS.ui.occurences";
	public final static String DECLARATION_ANNOTATION_ID = "SOS.resource.SOS.ui.occurences.declaration";
	
	private final static SOS.resource.SOS.ui.SOSPositionHelper positionHelper = new SOS.resource.SOS.ui.SOSPositionHelper();
	
	private SOS.resource.SOS.ui.SOSTokenScanner tokenScanner;
	 ...
	 */
	public SOSOccurrence(SOS.resource.SOS.ISOSTextResource textResource, org.eclipse.jface.text.source.projection.ProjectionViewer sourceViewer, SOS.resource.SOS.ui.SOSTokenScanner tokenScanner) {
		this.textResource = textResource;
		this.projectionViewer = sourceViewer;
		
		quotedTokenArray = new java.util.ArrayList<String>();
		String[] tokenNames = new SOS.resource.SOS.mopp.SOSMetaInformation().getTokenNames();
	 ...
	 */
	public void handleOccurrenceHighlighting(SOS.resource.SOS.ui.SOSBracketSet bracketSet) {
		...
		if (resource.equals(textResource)) {
			...
			while (!token.isEOF()) {
				...
				if (text.equals(tokenText)) {
					...
					addAnnotation(document, SOS.resource.SOS.ui.SOSPositionCategory.DEFINTION, text);
					break;
				}
				token = tokenScanner.nextToken();
			}
			...
			if (text != null && text.equals(tokenText) && tokenScanner.getTokenOffset() != defPosition) {
				...
				if (occEO != null) {
					if ((isNull && elementsAtDefinition.contains(occEO)) || !isNull && definitionElement.equals(occEO)) {
						addAnnotation(document, SOS.resource.SOS.ui.SOSPositionCategory.PROXY, text);
					}
				}
			}
			token = tokenScanner.nextToken();
	...
	private void addAnnotation(org.eclipse.jface.text.IDocument document, SOS.resource.SOS.ui.SOSPositionCategory type, String text) {
		int tokenOffset = tokenScanner.getTokenOffset();
		int tokenLength = tokenScanner.getTokenLength();
		...
		if (type == SOS.resource.SOS.ui.SOSPositionCategory.DEFINTION) {
	...
	private void removeAnnotations() {
		removeAnnotations(SOS.resource.SOS.ui.SOSOccurrence.OCCURRENCE_ANNOTATION_ID);
		removeAnnotations(SOS.resource.SOS.ui.SOSOccurrence.DECLARATION_ANNOTATION_ID);
	}
