 ...
 * </ul>
 */
public class DsltranstextBracketPreferencePage extends org.eclipse.jface.preference.PreferencePage implements org.eclipse.ui.IWorkbenchPreferencePage {
	
	private static final String[] ALL_LEFT_BRACKETS = new String[] { "{", "(", "[", "<", "\"", "'", };
	private static final String[] ALL_RIGHT_BRACKETS = new String[] { "}", ")", "]", ">", "\"", "'", };
	
	private String BRACKETS_COLOR = dsltrans.resource.dsltranstext.ui.DsltranstextPreferenceConstants.EDITOR_MATCHING_BRACKETS_COLOR;
	...
	private String language = new dsltrans.resource.dsltranstext.mopp.DsltranstextMetaInformation().getSyntaxName();
	
	private dsltrans.resource.dsltranstext.ui.DsltranstextBracketSet bracketsTmp;
	
	/**
	 * Creates a preference page for bracket setting.
	 */
	public DsltranstextBracketPreferencePage() {
		super();
		
		dsltrans.resource.dsltranstext.IDsltranstextMetaInformation metaInformation = new dsltrans.resource.dsltranstext.mopp.DsltranstextMetaInformation();
	...
	public void init(org.eclipse.ui.IWorkbench workbench) {
		setPreferenceStore(dsltrans.resource.dsltranstext.ui.DsltranstextUIPlugin.getDefault().getPreferenceStore());
		setDescription("Define the coloring of matching brackets.");
		
		bracketsTmp = new dsltrans.resource.dsltranstext.ui.DsltranstextBracketSet(null, null);
		for (String languageID : languageIDs) {
			bracketSetTemp.put(languageID, getPreferenceStore().getString(languageID + dsltrans.resource.dsltranstext.ui.DsltranstextPreferenceConstants.EDITOR_BRACKETS_SUFFIX));
		}
	}
	...
	private void handleMatchingBracketsSelection() {
		...
		enableCheckbox.setSelection(getPreferenceStore().getBoolean(		dsltrans.resource.dsltranstext.ui.DsltranstextPreferenceConstants.EDITOR_MATCHING_BRACKETS_CHECKBOX));
		enableClosingInside.setSelection(false);
		matchingBracketsColorButton.setEnabled(getPreferenceStore().getBoolean(		dsltrans.resource.dsltranstext.ui.DsltranstextPreferenceConstants.EDITOR_MATCHING_BRACKETS_CHECKBOX));
		...
		initializeLanguage();
		bracketsTmp.setBrackets(getPreferenceStore().getString(language + dsltrans.resource.dsltranstext.ui.DsltranstextPreferenceConstants.EDITOR_BRACKETS_SUFFIX));
		...
		bracketsList.addSelectionListener(new org.eclipse.swt.events.SelectionListener() {
			...
			public void widgetSelected(org.eclipse.swt.events.SelectionEvent e) {
				...
				for (int index : itemIndices) {
					dsltrans.resource.dsltranstext.IDsltranstextBracketPair bracketPair = bracketsTmp.getBracketPair(index);
		...
		enableClosingInside.addSelectionListener(new org.eclipse.swt.events.SelectionListener() {
			...
			public void widgetSelected(org.eclipse.swt.events.SelectionEvent e) {
				...
				for (int idx : itemIndices) {
					dsltrans.resource.dsltranstext.IDsltranstextBracketPair bracketPair = bracketsTmp.getBracketPair(idx);
	...
	protected void performDefaults() {
		enableCheckbox.setSelection(getPreferenceStore().getDefaultBoolean(dsltrans.resource.dsltranstext.ui.DsltranstextPreferenceConstants.EDITOR_MATCHING_BRACKETS_CHECKBOX));
		matchingBracketsColorButton.setEnabled(enableCheckbox.getSelection());
		...
		bracketSetTemp.put(language, getPreferenceStore().getDefaultString(language + dsltrans.resource.dsltranstext.ui.DsltranstextPreferenceConstants.EDITOR_BRACKETS_SUFFIX));
		bracketsTmp.setBrackets(bracketSetTemp.get(language));
		bracketsList.setItems(bracketsTmp.getBracketArray());
		enableClosingInside.setSelection(false);
	}
	...
	private void updateActiveEditor() {
		...
		getPreferenceStore().setValue(dsltrans.resource.dsltranstext.ui.DsltranstextPreferenceConstants.EDITOR_MATCHING_BRACKETS_CHECKBOX, enableCheckbox.getSelection());
		getPreferenceStore().setValue(language + dsltrans.resource.dsltranstext.ui.DsltranstextPreferenceConstants.EDITOR_BRACKETS_SUFFIX, bracketSetTemp.get(language));
		...
		if (editor != null && editor instanceof dsltrans.resource.dsltranstext.ui.DsltranstextEditor) {
			((dsltrans.resource.dsltranstext.ui.DsltranstextEditor) editor).invalidateTextRepresentation();
		}
	}
