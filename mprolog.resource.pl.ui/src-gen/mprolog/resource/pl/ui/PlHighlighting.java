 ...
 */
public class PlHighlighting implements org.eclipse.jface.viewers.ISelectionProvider, org.eclipse.jface.viewers.ISelectionChangedListener {
	
	private final static mprolog.resource.pl.ui.PlPositionHelper positionHelper = new mprolog.resource.pl.ui.PlPositionHelper();
	...
	private mprolog.resource.pl.ui.PlTokenScanner scanner;
	...
	private mprolog.resource.pl.ui.PlEditor editor;
	private org.eclipse.jface.text.source.projection.ProjectionViewer projectionViewer;
	private mprolog.resource.pl.ui.PlOccurrence occurrence;
	private mprolog.resource.pl.ui.PlBracketSet bracketSet;
	 ...
	 */
	private final class UpdateHighlightingListener implements org.eclipse.swt.events.KeyListener, org.eclipse.swt.events.VerifyListener, org.eclipse.swt.events.MouseListener, mprolog.resource.pl.IPlBackgroundParsingListener {
	 ...
	 * @param editor
	 */
	public PlHighlighting(mprolog.resource.pl.IPlTextResource textResource, org.eclipse.jface.text.source.projection.ProjectionViewer sourceviewer, mprolog.resource.pl.ui.PlColorManager colorManager, mprolog.resource.pl.ui.PlEditor editor) {
		this.display = org.eclipse.swt.widgets.Display.getCurrent();
		sourceviewer.getSelectionProvider();
		preferenceStore = mprolog.resource.pl.ui.PlUIPlugin.getDefault().getPreferenceStore();
		this.editor = editor;
		textWidget = sourceviewer.getTextWidget();
		projectionViewer = sourceviewer;
		scanner = new mprolog.resource.pl.ui.PlTokenScanner(textResource, colorManager);
		occurrence = new mprolog.resource.pl.ui.PlOccurrence(textResource, sourceviewer, scanner);
		bracketSet = new mprolog.resource.pl.ui.PlBracketSet(editor, sourceviewer);
		this.colorManager = colorManager;
		isHighlightBrackets = preferenceStore.getBoolean(mprolog.resource.pl.ui.PlPreferenceConstants.EDITOR_MATCHING_BRACKETS_CHECKBOX);
		bracketColor = colorManager.getColor(org.eclipse.jface.preference.PreferenceConverter.getColor(preferenceStore, mprolog.resource.pl.ui.PlPreferenceConstants.EDITOR_MATCHING_BRACKETS_COLOR));
		black = colorManager.getColor(new org.eclipse.swt.graphics.RGB(0, 0, 0));
		
		addListeners(editor);
	}
	
	private void addListeners(mprolog.resource.pl.ui.PlEditor editor) {
	...
	private void setBracketHighlighting(org.eclipse.jface.text.IDocument document) {
		...
		org.eclipse.jface.text.Position[] positions = positionHelper.getPositions(document, mprolog.resource.pl.ui.PlPositionCategory.BRACKET.toString());
	...
	private void removeHighlighting() {
		...
		removeHighlightingCategory(document, mprolog.resource.pl.ui.PlPositionCategory.BRACKET.toString());
	}
	...
	private void removeHighlightingCategory(org.eclipse.jface.text.IDocument document, String category) {
		...
		if (category.equals(mprolog.resource.pl.ui.PlPositionCategory.BRACKET.toString())) {
		...
		display.syncExec(new Runnable() {
			public void run() {
				...
				if (selectedEObject != null) {
					setSelection(new mprolog.resource.pl.ui.PlEObjectSelection(selectedEObject, false));
				}
			}
		});
	}
