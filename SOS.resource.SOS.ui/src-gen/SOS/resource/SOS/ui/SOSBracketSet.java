...
public class SOSBracketSet {
	...
	private java.util.ArrayList<SOS.resource.SOS.ISOSBracketPair> bracketPairs;
	 ...
	 */
	private class BracketPair implements SOS.resource.SOS.ISOSBracketPair {
	 ...
	 */
	private class ClosingListener implements SOS.resource.SOS.ui.ISOSBracketHandler, org.eclipse.swt.events.VerifyListener, org.eclipse.swt.events.ModifyListener, org.eclipse.swt.custom.VerifyKeyListener {
		...
		public void verifyText(org.eclipse.swt.events.VerifyEvent e) {
			...
			if (caret > 0 && caret < textWidget.getCharCount()) {
				SOS.resource.SOS.ISOSBracketPair bracketPair = getBracketPair(textWidget.getTextRange(caret - 1, 1), textWidget.getTextRange(caret, 1));
	 ...
	 */
	public SOSBracketSet(SOS.resource.SOS.ui.SOSEditor editor, org.eclipse.jface.text.source.ISourceViewer sourceViewer) {
		languageID = new SOS.resource.SOS.mopp.SOSMetaInformation().getSyntaxName();
		this.bracketPairs = new java.util.ArrayList<SOS.resource.SOS.ISOSBracketPair>();
	 ...
	 */
	public SOS.resource.SOS.ISOSBracketPair getBracketPair(String opening, String closing) {
		for (SOS.resource.SOS.ISOSBracketPair bracketPair : bracketPairs) {
	...
	public SOS.resource.SOS.ISOSBracketPair getBracketPair(int index) {
		try {
	 ...
	 */
	public boolean setClosingEnabledInside(SOS.resource.SOS.ISOSBracketPair bracketPair, boolean closingEnabledInside) {
	...
	public boolean resetBrackets() {
		String bracketPairs = preferenceStore.getString(languageID + SOS.resource.SOS.ui.SOSPreferenceConstants.EDITOR_BRACKETS_SUFFIX);
	 ...
	 */
	public SOS.resource.SOS.ISOSBracketPair remove(String opening, String closing) {
		for (SOS.resource.SOS.ISOSBracketPair bracketPair : bracketPairs) {
			if (bracketPair.getOpeningBracket().equals(opening) && bracketPair.getClosingBracket().equals(closing)) {
	...
	public boolean setBrackets(String bracketSet) {
		...
		bracketPairs = new java.util.ArrayList<SOS.resource.SOS.ISOSBracketPair>();
	...
	public void matchingBrackets() {
		...
		if (position != -1 && position != document.getLength()) {
			positionHelper.addPosition(document, SOS.resource.SOS.ui.SOSPositionCategory.BRACKET.toString(), position, 1);
			positionHelper.addPosition(document, SOS.resource.SOS.ui.SOSPositionCategory.BRACKET.toString(), caretOffset - 1, 1);
		}
	}
