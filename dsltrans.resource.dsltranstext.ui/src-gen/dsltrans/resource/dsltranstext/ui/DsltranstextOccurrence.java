 ...
 */
public class DsltranstextOccurrence {
	
	public final static String OCCURRENCE_ANNOTATION_ID = "dsltrans.resource.dsltranstext.ui.occurences";
	public final static String DECLARATION_ANNOTATION_ID = "dsltrans.resource.dsltranstext.ui.occurences.declaration";
	...
	private dsltrans.resource.dsltranstext.ui.DsltranstextTokenScanner tokenScanner;
	...
	private dsltrans.resource.dsltranstext.IDsltranstextTextResource textResource;
	 ...
	 */
	public DsltranstextOccurrence(dsltrans.resource.dsltranstext.IDsltranstextTextResource textResource, org.eclipse.jface.text.source.projection.ProjectionViewer sourceViewer, dsltrans.resource.dsltranstext.ui.DsltranstextTokenScanner tokenScanner) {
		this.textResource = textResource;
		this.projectionViewer = sourceViewer;
		
		quotedTokenArray = new java.util.ArrayList<String>();
		String[] tokenNames = new dsltrans.resource.dsltranstext.mopp.DsltranstextMetaInformation().getTokenNames();
	 ...
	 */
	public void handleOccurrenceHighlighting(dsltrans.resource.dsltranstext.ui.DsltranstextBracketSet bracketSet) {
		...
		if (resource.equals(textResource)) {
			...
			while (!token.isEOF()) {
				...
				if (text.equals(tokenText)) {
					...
					addAnnotation(document, dsltrans.resource.dsltranstext.ui.DsltranstextPositionCategory.DEFINTION, text);
					break;
				}
				token = tokenScanner.nextToken();
			}
			...
			if (text != null && text.equals(tokenText) && tokenScanner.getTokenOffset() != defPosition) {
				...
				if (occEO != null) {
					if ((isNull && elementsAtDefinition.contains(occEO)) || !isNull && definitionElement.equals(occEO)) {
						addAnnotation(document, dsltrans.resource.dsltranstext.ui.DsltranstextPositionCategory.PROXY, text);
					}
				}
			}
			token = tokenScanner.nextToken();
	...
	private void addAnnotation(org.eclipse.jface.text.IDocument document, dsltrans.resource.dsltranstext.ui.DsltranstextPositionCategory type, String text) {
		int tokenOffset = tokenScanner.getTokenOffset();
		int tokenLength = tokenScanner.getTokenLength();
		...
		if (type == dsltrans.resource.dsltranstext.ui.DsltranstextPositionCategory.DEFINTION) {
	...
	private void removeAnnotations() {
		removeAnnotations(dsltrans.resource.dsltranstext.ui.DsltranstextOccurrence.OCCURRENCE_ANNOTATION_ID);
		removeAnnotations(dsltrans.resource.dsltranstext.ui.DsltranstextOccurrence.DECLARATION_ANNOTATION_ID);
	}
