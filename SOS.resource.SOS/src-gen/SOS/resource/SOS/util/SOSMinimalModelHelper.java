 ...
 */
public class SOSMinimalModelHelper {
	
	private final static SOS.resource.SOS.util.SOSEClassUtil eClassUtil = new SOS.resource.SOS.util.SOSEClassUtil();
			...
			if (feature instanceof org.eclipse.emf.ecore.EReference) {
				...
				if (type instanceof org.eclipse.emf.ecore.EClass) {
					...
					for (int i = 0; i < lowerBound; i++) {
						...
						if (reference.isContainment()) {
							...
							String initialValue = "#some" + SOS.resource.SOS.util.SOSStringUtil.capitalize(typeClass.getName());
						...
						if (value instanceof java.util.List<?>) {
							java.util.List<org.eclipse.emf.ecore.EObject> list = SOS.resource.SOS.util.SOSListUtil.castListUnchecked(value);
							list.add(subModel);
						} else {
				...
				if ("EString".equals(attribute.getEType().getName())) {
					...
					if (attribute.getName().equals("name") && name != null) {
						...
						initialValue = "some" + SOS.resource.SOS.util.SOSStringUtil.capitalize(attribute.getName());
					}
					Object value = root.eGet(attribute);
					if (value instanceof java.util.List<?>) {
						java.util.List<String> list = SOS.resource.SOS.util.SOSListUtil.castListUnchecked(value);
