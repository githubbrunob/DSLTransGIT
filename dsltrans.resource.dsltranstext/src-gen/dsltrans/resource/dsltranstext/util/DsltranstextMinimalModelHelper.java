 ...
 */
public class DsltranstextMinimalModelHelper {
	
	private final static dsltrans.resource.dsltranstext.util.DsltranstextEClassUtil eClassUtil = new dsltrans.resource.dsltranstext.util.DsltranstextEClassUtil();
			...
			if (feature instanceof org.eclipse.emf.ecore.EReference) {
				...
				if (type instanceof org.eclipse.emf.ecore.EClass) {
					...
					for (int i = 0; i < lowerBound; i++) {
						...
						if (reference.isContainment()) {
							...
							String initialValue = "#some" + dsltrans.resource.dsltranstext.util.DsltranstextStringUtil.capitalize(typeClass.getName());
						...
						if (value instanceof java.util.List<?>) {
							java.util.List<org.eclipse.emf.ecore.EObject> list = dsltrans.resource.dsltranstext.util.DsltranstextListUtil.castListUnchecked(value);
							list.add(subModel);
						} else {
				...
				if ("EString".equals(attribute.getEType().getName())) {
					...
					if (attribute.getName().equals("name") && name != null) {
						...
						initialValue = "some" + dsltrans.resource.dsltranstext.util.DsltranstextStringUtil.capitalize(attribute.getName());
					}
					Object value = root.eGet(attribute);
					if (value instanceof java.util.List<?>) {
						java.util.List<String> list = dsltrans.resource.dsltranstext.util.DsltranstextListUtil.castListUnchecked(value);
