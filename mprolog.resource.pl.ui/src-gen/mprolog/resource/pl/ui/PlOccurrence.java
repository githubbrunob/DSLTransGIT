 ...
 */
public class PlOccurrence {
	
	public final static String OCCURRENCE_ANNOTATION_ID = "mprolog.resource.pl.ui.occurences";
	public final static String DECLARATION_ANNOTATION_ID = "mprolog.resource.pl.ui.occurences.declaration";
	
	private final static mprolog.resource.pl.ui.PlPositionHelper positionHelper = new mprolog.resource.pl.ui.PlPositionHelper();
	
	private mprolog.resource.pl.ui.PlTokenScanner tokenScanner;
	 ...
	 */
	public PlOccurrence(mprolog.resource.pl.IPlTextResource textResource, org.eclipse.jface.text.source.projection.ProjectionViewer sourceViewer, mprolog.resource.pl.ui.PlTokenScanner tokenScanner) {
		this.textResource = textResource;
		this.projectionViewer = sourceViewer;
		
		quotedTokenArray = new java.util.ArrayList<String>();
		String[] tokenNames = new mprolog.resource.pl.mopp.PlMetaInformation().getTokenNames();
	 ...
	 */
	public void handleOccurrenceHighlighting(mprolog.resource.pl.ui.PlBracketSet bracketSet) {
		...
		if (resource.equals(textResource)) {
			...
			while (!token.isEOF()) {
				...
				if (text.equals(tokenText)) {
					...
					addAnnotation(document, mprolog.resource.pl.ui.PlPositionCategory.DEFINTION, text);
					break;
				}
				token = tokenScanner.nextToken();
			}
			...
			if (text != null && text.equals(tokenText) && tokenScanner.getTokenOffset() != defPosition) {
				...
				if (occEO != null) {
					if ((isNull && elementsAtDefinition.contains(occEO)) || !isNull && definitionElement.equals(occEO)) {
						addAnnotation(document, mprolog.resource.pl.ui.PlPositionCategory.PROXY, text);
					}
				}
			}
			token = tokenScanner.nextToken();
	...
	private void addAnnotation(org.eclipse.jface.text.IDocument document, mprolog.resource.pl.ui.PlPositionCategory type, String text) {
		int tokenOffset = tokenScanner.getTokenOffset();
		int tokenLength = tokenScanner.getTokenLength();
		...
		if (type == mprolog.resource.pl.ui.PlPositionCategory.DEFINTION) {
	...
	private void removeAnnotations() {
		removeAnnotations(mprolog.resource.pl.ui.PlOccurrence.OCCURRENCE_ANNOTATION_ID);
		removeAnnotations(mprolog.resource.pl.ui.PlOccurrence.DECLARATION_ANNOTATION_ID);
	}
