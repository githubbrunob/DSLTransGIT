 ...
 * contains <code>SOS.resource.SOS.ISOSTextDiagnostic</code>s it uses the more
 * precise information of this extended diagnostic type.
 */
public class SOSMarkerHelper {
	
	/**
	 * The extension id of the custom marker type that is used by this text resource.
	 */
	public static final String MARKER_TYPE = SOS.resource.SOS.mopp.SOSPlugin.PLUGIN_ID + ".problem";
	...
	private static class MarkerCommandQueue {
		...
		private java.util.List<SOS.resource.SOS.ISOSCommand<Object>> commands = new java.util.ArrayList<SOS.resource.SOS.ISOSCommand<Object>>();
		private final Object jobLock = new Object();
		
		public void addCommand(SOS.resource.SOS.ISOSCommand<Object> command) {
			...
			new org.eclipse.core.runtime.jobs.Job("updating markers") {
				...
				protected org.eclipse.core.runtime.IStatus run(org.eclipse.core.runtime.IProgressMonitor monitor) {
					synchronized(jobLock) {
						java.util.List<SOS.resource.SOS.ISOSCommand<Object>> commandsToProcess = new java.util.ArrayList<SOS.resource.SOS.ISOSCommand<Object>>();
						synchronized(commands) {
							commandsToProcess.addAll(commands);
							commands.clear();
						}
						for (SOS.resource.SOS.ISOSCommand<Object> command : commandsToProcess) {
	 ...
	 */
	public static void mark(org.eclipse.emf.ecore.resource.Resource resource, final SOS.resource.SOS.ISOSTextDiagnostic diagnostic) {
	...
	private static void createMarkerFromDiagnostic(final org.eclipse.core.resources.IFile file, final SOS.resource.SOS.ISOSTextDiagnostic diagnostic) {
		final SOS.resource.SOS.ISOSProblem problem = diagnostic.getProblem();
		SOS.resource.SOS.SOSEProblemType problemType = problem.getType();
		final String markerID = getMarkerID(problemType);
		COMMAND_QUEUE.addCommand(new SOS.resource.SOS.ISOSCommand<Object>() {
			public boolean execute(Object context) {
				try {
					...
					if (problem.getSeverity() == SOS.resource.SOS.SOSEProblemSeverity.ERROR) {
					...
					SOS.resource.SOS.ISOSTextDiagnostic textDiagnostic = (SOS.resource.SOS.ISOSTextDiagnostic) diagnostic;
					...
					if (diagnostic instanceof SOS.resource.SOS.mopp.SOSResource.ElementBasedTextDiagnostic) {
						org.eclipse.emf.ecore.EObject element = ((SOS.resource.SOS.mopp.SOSResource.ElementBasedTextDiagnostic) diagnostic).getElement();
					...
					java.util.Collection<SOS.resource.SOS.ISOSQuickFix> quickFixes = textDiagnostic.getProblem().getQuickFixes();
					...
					if (!sourceIDs.isEmpty()) {
						marker.setAttribute(org.eclipse.core.resources.IMarker.SOURCE_ID, SOS.resource.SOS.util.SOSStringUtil.explode(sourceIDs, "|"));
					}
	...
	public static void unmark(org.eclipse.emf.ecore.resource.Resource resource) {
		for (SOS.resource.SOS.SOSEProblemType nextType : SOS.resource.SOS.SOSEProblemType.values()) {
	 ...
	 */
	public static void unmark(org.eclipse.emf.ecore.resource.Resource resource, SOS.resource.SOS.SOSEProblemType problemType) {
		final org.eclipse.core.resources.IFile file = getFile(resource);
		if (file == null) {
			return;
		}
	...
	public static void unmark(org.eclipse.emf.ecore.resource.Resource resource, final org.eclipse.emf.ecore.EObject causingObject) {
		...
		final String markerID = getMarkerID(SOS.resource.SOS.SOSEProblemType.UNKNOWN);
	 ...
	 * given type.
	 */
	private static String getMarkerID(SOS.resource.SOS.SOSEProblemType problemType) {
	...
	private static void handleException(org.eclipse.core.runtime.CoreException ce) {
		if (ce.getMessage().matches("Marker.*not found.")) {
			...
			SOS.resource.SOS.mopp.SOSPlugin.logError("Error while removing markers from resource:", ce);
		}
	}
}
