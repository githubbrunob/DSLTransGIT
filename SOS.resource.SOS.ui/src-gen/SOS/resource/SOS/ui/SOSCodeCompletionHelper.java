 ...
 */
public class SOSCodeCompletionHelper {
	
	private SOS.resource.SOS.mopp.SOSAttributeValueProvider attributeValueProvider = new SOS.resource.SOS.mopp.SOSAttributeValueProvider();
	 ...
	 * @return
	 */
	public SOS.resource.SOS.ui.SOSCompletionProposal[] computeCompletionProposals(SOS.resource.SOS.ISOSTextResource originalResource, String content, int cursorOffset) {
		...
		SOS.resource.SOS.ISOSTextResource resource = (SOS.resource.SOS.ISOSTextResource) resourceSet.createResource(originalResource.getURI());
		java.io.ByteArrayInputStream inputStream = new java.io.ByteArrayInputStream(content.getBytes());
		SOS.resource.SOS.ISOSMetaInformation metaInformation = resource.getMetaInformation();
		SOS.resource.SOS.ISOSTextParser parser = metaInformation.createParser(inputStream, null);
		SOS.resource.SOS.mopp.SOSExpectedTerminal[] expectedElements = parseToExpectedElements(parser, resource, cursorOffset);
		if (expectedElements == null) {
			return new SOS.resource.SOS.ui.SOSCompletionProposal[0];
		}
		if (expectedElements.length == 0) {
			return new SOS.resource.SOS.ui.SOSCompletionProposal[0];
		}
		java.util.List<SOS.resource.SOS.mopp.SOSExpectedTerminal> expectedAfterCursor = java.util.Arrays.asList(getElementsExpectedAt(expectedElements, cursorOffset));
		java.util.List<SOS.resource.SOS.mopp.SOSExpectedTerminal> expectedBeforeCursor = java.util.Arrays.asList(getElementsExpectedAt(expectedElements, cursorOffset - 1));
		...
		java.util.Collection<SOS.resource.SOS.ui.SOSCompletionProposal> allProposals = new java.util.LinkedHashSet<SOS.resource.SOS.ui.SOSCompletionProposal>();
		java.util.Collection<SOS.resource.SOS.ui.SOSCompletionProposal> rightProposals = deriveProposals(expectedAfterCursor, content, resource, cursorOffset);
		java.util.Collection<SOS.resource.SOS.ui.SOSCompletionProposal> leftProposals = deriveProposals(expectedBeforeCursor, content, resource, cursorOffset - 1);
		...
		final java.util.List<SOS.resource.SOS.ui.SOSCompletionProposal> sortedProposals = new java.util.ArrayList<SOS.resource.SOS.ui.SOSCompletionProposal>(allProposals);
		java.util.Collections.sort(sortedProposals);
		return sortedProposals.toArray(new SOS.resource.SOS.ui.SOSCompletionProposal[sortedProposals.size()]);
	}
	
	public SOS.resource.SOS.mopp.SOSExpectedTerminal[] parseToExpectedElements(SOS.resource.SOS.ISOSTextParser parser, SOS.resource.SOS.ISOSTextResource resource, int cursorOffset) {
		final java.util.List<SOS.resource.SOS.mopp.SOSExpectedTerminal> expectedElements = parser.parseToExpectedElements(null, resource, cursorOffset);
		if (expectedElements == null) {
			return new SOS.resource.SOS.mopp.SOSExpectedTerminal[0];
		}
		removeDuplicateEntries(expectedElements);
		removeInvalidEntriesAtEnd(expectedElements);
		return expectedElements.toArray(new SOS.resource.SOS.mopp.SOSExpectedTerminal[expectedElements.size()]);
	}
	
	private void removeDuplicateEntries(java.util.List<SOS.resource.SOS.mopp.SOSExpectedTerminal> expectedElements) {
		for (int i = 0; i < expectedElements.size() - 1; i++) {
			SOS.resource.SOS.mopp.SOSExpectedTerminal elementAtIndex = expectedElements.get(i);
			for (int j = i + 1; j < expectedElements.size();) {
				SOS.resource.SOS.mopp.SOSExpectedTerminal elementAtNext = expectedElements.get(j);
	...
	private void removeInvalidEntriesAtEnd(java.util.List<SOS.resource.SOS.mopp.SOSExpectedTerminal> expectedElements) {
		for (int i = 0; i < expectedElements.size() - 1;) {
			SOS.resource.SOS.mopp.SOSExpectedTerminal elementAtIndex = expectedElements.get(i);
			SOS.resource.SOS.mopp.SOSExpectedTerminal elementAtNext = expectedElements.get(i + 1);
	...
	private String findPrefix(java.util.List<SOS.resource.SOS.mopp.SOSExpectedTerminal> expectedElements, SOS.resource.SOS.mopp.SOSExpectedTerminal expectedAtCursor, String content, int cursorOffset) {
		if (cursorOffset < 0) {
			return "";
		}
		int end = 0;
		for (SOS.resource.SOS.mopp.SOSExpectedTerminal expectedElement : expectedElements) {
	...
	private java.util.Collection<SOS.resource.SOS.ui.SOSCompletionProposal> deriveProposals(java.util.List<SOS.resource.SOS.mopp.SOSExpectedTerminal> expectedElements, String content, SOS.resource.SOS.ISOSTextResource resource, int cursorOffset) {
		java.util.Collection<SOS.resource.SOS.ui.SOSCompletionProposal> resultSet = new java.util.LinkedHashSet<SOS.resource.SOS.ui.SOSCompletionProposal>();
		for (SOS.resource.SOS.mopp.SOSExpectedTerminal expectedElement : expectedElements) {
	...
	private java.util.Collection<SOS.resource.SOS.ui.SOSCompletionProposal> deriveProposals(SOS.resource.SOS.mopp.SOSExpectedTerminal expectedTerminal, String content, SOS.resource.SOS.ISOSTextResource resource, int cursorOffset) {
		SOS.resource.SOS.ISOSMetaInformation metaInformation = resource.getMetaInformation();
		SOS.resource.SOS.ISOSLocationMap locationMap = resource.getLocationMap();
		SOS.resource.SOS.ISOSExpectedElement expectedElement = (SOS.resource.SOS.ISOSExpectedElement) expectedTerminal.getTerminal();
		if (expectedElement instanceof SOS.resource.SOS.mopp.SOSExpectedCsString) {
			SOS.resource.SOS.mopp.SOSExpectedCsString csString = (SOS.resource.SOS.mopp.SOSExpectedCsString) expectedElement;
			return handleKeyword(csString, expectedTerminal.getPrefix());
		} else if (expectedElement instanceof SOS.resource.SOS.mopp.SOSExpectedBooleanTerminal) {
			SOS.resource.SOS.mopp.SOSExpectedBooleanTerminal expectedBooleanTerminal = (SOS.resource.SOS.mopp.SOSExpectedBooleanTerminal) expectedElement;
			return handleBooleanTerminal(expectedBooleanTerminal, expectedTerminal.getPrefix());
		} else if (expectedElement instanceof SOS.resource.SOS.mopp.SOSExpectedEnumerationTerminal) {
			SOS.resource.SOS.mopp.SOSExpectedEnumerationTerminal expectedEnumerationTerminal = (SOS.resource.SOS.mopp.SOSExpectedEnumerationTerminal) expectedElement;
			return handleEnumerationTerminal(expectedEnumerationTerminal, expectedTerminal.getPrefix());
		} else if (expectedElement instanceof SOS.resource.SOS.mopp.SOSExpectedStructuralFeature) {
			SOS.resource.SOS.mopp.SOSExpectedStructuralFeature expectedFeature = (SOS.resource.SOS.mopp.SOSExpectedStructuralFeature) expectedElement;
					...
					for (int i = contentList.size() - 1; i >= 0; i--) {
						...
						if (neededClass.isInstance(object)) {
							...
							if (eStructuralFeature.getEType().isInstance(newContainer)) {
								SOS.resource.SOS.util.SOSEObjectUtil.setFeature(object, eStructuralFeature, newContainer, false);
	...
	private java.util.Collection<SOS.resource.SOS.ui.SOSCompletionProposal> handleEnumAttribute(SOS.resource.SOS.ISOSMetaInformation metaInformation, SOS.resource.SOS.mopp.SOSExpectedStructuralFeature expectedFeature, org.eclipse.emf.ecore.EEnum enumType, String prefix, org.eclipse.emf.ecore.EObject container) {
		java.util.Collection<org.eclipse.emf.ecore.EEnumLiteral> enumLiterals = enumType.getELiterals();
		java.util.Collection<SOS.resource.SOS.ui.SOSCompletionProposal> result = new java.util.LinkedHashSet<SOS.resource.SOS.ui.SOSCompletionProposal>();
		for (org.eclipse.emf.ecore.EEnumLiteral literal : enumLiterals) {
			...
			SOS.resource.SOS.ISOSTokenResolverFactory tokenResolverFactory = metaInformation.getTokenResolverFactory();
			SOS.resource.SOS.ISOSTokenResolver tokenResolver = tokenResolverFactory.createTokenResolver(expectedFeature.getTokenName());
			...
			result.add(new SOS.resource.SOS.ui.SOSCompletionProposal(resolvedLiteral, prefix, matchesPrefix, expectedFeature.getFeature(), container));
		}
		return result;
	}
	
	private java.util.Collection<SOS.resource.SOS.ui.SOSCompletionProposal> handleNCReference(SOS.resource.SOS.ISOSMetaInformation metaInformation, org.eclipse.emf.ecore.EObject container, org.eclipse.emf.ecore.EReference reference, String prefix, String tokenName) {
		// proposals for non-containment references are derived by calling the reference
		// resolver switch in fuzzy mode.
		SOS.resource.SOS.ISOSReferenceResolverSwitch resolverSwitch = metaInformation.getReferenceResolverSwitch();
		SOS.resource.SOS.ISOSTokenResolverFactory tokenResolverFactory = metaInformation.getTokenResolverFactory();
		SOS.resource.SOS.ISOSReferenceResolveResult<org.eclipse.emf.ecore.EObject> result = new SOS.resource.SOS.mopp.SOSReferenceResolveResult<org.eclipse.emf.ecore.EObject>(true);
		resolverSwitch.resolveFuzzy(prefix, container, reference, 0, result);
		java.util.Collection<SOS.resource.SOS.ISOSReferenceMapping<org.eclipse.emf.ecore.EObject>> mappings = result.getMappings();
		if (mappings != null) {
			java.util.Collection<SOS.resource.SOS.ui.SOSCompletionProposal> resultSet = new java.util.LinkedHashSet<SOS.resource.SOS.ui.SOSCompletionProposal>();
			for (SOS.resource.SOS.ISOSReferenceMapping<org.eclipse.emf.ecore.EObject> mapping : mappings) {
				org.eclipse.swt.graphics.Image image = null;
				if (mapping instanceof SOS.resource.SOS.mopp.SOSElementMapping<?>) {
					SOS.resource.SOS.mopp.SOSElementMapping<?> elementMapping = (SOS.resource.SOS.mopp.SOSElementMapping<?>) mapping;
					...
					resultSet.add(new SOS.resource.SOS.ui.SOSCompletionProposal(identifier, prefix, matchesPrefix, reference, container, image));
				}
			}
			return resultSet;
		}
	...
	private java.util.Collection<SOS.resource.SOS.ui.SOSCompletionProposal> handleAttribute(SOS.resource.SOS.ISOSMetaInformation metaInformation, SOS.resource.SOS.mopp.SOSExpectedStructuralFeature expectedFeature, org.eclipse.emf.ecore.EObject container, org.eclipse.emf.ecore.EAttribute attribute, String prefix) {
		java.util.Collection<SOS.resource.SOS.ui.SOSCompletionProposal> resultSet = new java.util.LinkedHashSet<SOS.resource.SOS.ui.SOSCompletionProposal>();
				...
				if (defaultValue != null) {
					...
					if (tokenName != null) {
						...
						if (tokenResolver != null) {
							...
							resultSet.add(new SOS.resource.SOS.ui.SOSCompletionProposal(defaultValueAsString, prefix, matchesPrefix, expectedFeature.getFeature(), container));
						}
					}
				}
			}
	...
	private java.util.Collection<SOS.resource.SOS.ui.SOSCompletionProposal> handleKeyword(SOS.resource.SOS.mopp.SOSExpectedCsString csString, String prefix) {
		String proposal = csString.getValue();
		boolean matchesPrefix = matches(proposal, prefix);
		return java.util.Collections.singleton(new SOS.resource.SOS.ui.SOSCompletionProposal(proposal, prefix, matchesPrefix, null, null));
	}
	
	private java.util.Collection<SOS.resource.SOS.ui.SOSCompletionProposal> handleBooleanTerminal(SOS.resource.SOS.mopp.SOSExpectedBooleanTerminal expectedBooleanTerminal, String prefix) {
		java.util.Collection<SOS.resource.SOS.ui.SOSCompletionProposal> result = new java.util.LinkedHashSet<SOS.resource.SOS.ui.SOSCompletionProposal>(2);
		SOS.resource.SOS.grammar.SOSBooleanTerminal booleanTerminal = expectedBooleanTerminal.getBooleanTerminal();
	...
	private java.util.Collection<SOS.resource.SOS.ui.SOSCompletionProposal> handleEnumerationTerminal(SOS.resource.SOS.mopp.SOSExpectedEnumerationTerminal expectedEnumerationTerminal, String prefix) {
		java.util.Collection<SOS.resource.SOS.ui.SOSCompletionProposal> result = new java.util.LinkedHashSet<SOS.resource.SOS.ui.SOSCompletionProposal>(2);
		SOS.resource.SOS.grammar.SOSEnumerationTerminal enumerationTerminal = expectedEnumerationTerminal.getEnumerationTerminal();
	...
	private java.util.Collection<SOS.resource.SOS.ui.SOSCompletionProposal> handleLiteral(org.eclipse.emf.ecore.EAttribute attribute, String prefix, String literal) {
		if ("".equals(literal)) {
			return java.util.Collections.emptySet();
		}
		boolean matchesPrefix = matches(literal, prefix);
		return java.util.Collections.singleton(new SOS.resource.SOS.ui.SOSCompletionProposal(literal, prefix, matchesPrefix, null, null));
	}
	 ...
	 * element is expected.
	 */
	private void setPrefixes(java.util.List<SOS.resource.SOS.mopp.SOSExpectedTerminal> expectedElements, String content, int cursorOffset) {
		if (cursorOffset < 0) {
			return;
		}
		for (SOS.resource.SOS.mopp.SOSExpectedTerminal expectedElement : expectedElements) {
	...
	public SOS.resource.SOS.mopp.SOSExpectedTerminal[] getElementsExpectedAt(SOS.resource.SOS.mopp.SOSExpectedTerminal[] allExpectedElements, int cursorOffset) {
		java.util.List<SOS.resource.SOS.mopp.SOSExpectedTerminal> expectedAtCursor = new java.util.ArrayList<SOS.resource.SOS.mopp.SOSExpectedTerminal>();
		for (int i = 0; i < allExpectedElements.length; i++) {
			SOS.resource.SOS.mopp.SOSExpectedTerminal expectedElement = allExpectedElements[i];
		...
		return expectedAtCursor.toArray(new SOS.resource.SOS.mopp.SOSExpectedTerminal[expectedAtCursor.size()]);
	}
	
	private int getEnd(SOS.resource.SOS.mopp.SOSExpectedTerminal[] allExpectedElements, int indexInList) {
		SOS.resource.SOS.mopp.SOSExpectedTerminal elementAtIndex = allExpectedElements[indexInList];
		...
		for (int i = indexInList + 1; i < allExpectedElements.length; i++) {
			SOS.resource.SOS.mopp.SOSExpectedTerminal elementAtI = allExpectedElements[i];
	...
	private boolean matches(String proposal, String prefix) {
		return (proposal.toLowerCase().startsWith(prefix.toLowerCase()) || SOS.resource.SOS.util.SOSStringUtil.matchCamelCase(prefix, proposal) != null) && !proposal.equals(prefix);
	}
