...
public class PlBracketSet {
	...
	private java.util.ArrayList<mprolog.resource.pl.IPlBracketPair> bracketPairs;
	 ...
	 */
	private class BracketPair implements mprolog.resource.pl.IPlBracketPair {
	 ...
	 */
	private class ClosingListener implements mprolog.resource.pl.ui.IPlBracketHandler, org.eclipse.swt.events.VerifyListener, org.eclipse.swt.events.ModifyListener, org.eclipse.swt.custom.VerifyKeyListener {
		...
		public void verifyText(org.eclipse.swt.events.VerifyEvent e) {
			...
			if (caret > 0 && caret < textWidget.getCharCount()) {
				mprolog.resource.pl.IPlBracketPair bracketPair = getBracketPair(textWidget.getTextRange(caret - 1, 1), textWidget.getTextRange(caret, 1));
	 ...
	 */
	public PlBracketSet(mprolog.resource.pl.ui.PlEditor editor, org.eclipse.jface.text.source.ISourceViewer sourceViewer) {
		languageID = new mprolog.resource.pl.mopp.PlMetaInformation().getSyntaxName();
		this.bracketPairs = new java.util.ArrayList<mprolog.resource.pl.IPlBracketPair>();
	 ...
	 */
	public mprolog.resource.pl.IPlBracketPair getBracketPair(String opening, String closing) {
		for (mprolog.resource.pl.IPlBracketPair bracketPair : bracketPairs) {
	...
	public mprolog.resource.pl.IPlBracketPair getBracketPair(int index) {
		try {
	 ...
	 */
	public boolean setClosingEnabledInside(mprolog.resource.pl.IPlBracketPair bracketPair, boolean closingEnabledInside) {
	...
	public boolean resetBrackets() {
		String bracketPairs = preferenceStore.getString(languageID + mprolog.resource.pl.ui.PlPreferenceConstants.EDITOR_BRACKETS_SUFFIX);
	 ...
	 */
	public mprolog.resource.pl.IPlBracketPair remove(String opening, String closing) {
		for (mprolog.resource.pl.IPlBracketPair bracketPair : bracketPairs) {
			if (bracketPair.getOpeningBracket().equals(opening) && bracketPair.getClosingBracket().equals(closing)) {
	...
	public boolean setBrackets(String bracketSet) {
		...
		bracketPairs = new java.util.ArrayList<mprolog.resource.pl.IPlBracketPair>();
	...
	public void matchingBrackets() {
		...
		if (position != -1 && position != document.getLength()) {
			positionHelper.addPosition(document, mprolog.resource.pl.ui.PlPositionCategory.BRACKET.toString(), position, 1);
			positionHelper.addPosition(document, mprolog.resource.pl.ui.PlPositionCategory.BRACKET.toString(), caretOffset - 1, 1);
		}
	}
